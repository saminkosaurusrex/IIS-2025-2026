...
function selectBlockedTimes() {
    ...
    const blocked = props.events.filter(
        (event) =>
            event.hall_id === form.hall &&
            !(
                event.hall_id == props.event.hall_id &&
                event.show_id == props.event.show_id
            ),
    );
...
const isBlocked = (hour: string) => {
    ...
    return blockedForDate.some(
        ({ start_at, end_at }) => hour >= start_at && hour < end_at,
    );
};
        ...
        <div class="p-4">
            <form @submit.prevent="handleSubmit" class="w-8/12 space-y-4">
                <div class="space-y-2">
                    ...
                    <div class="text-sm text-red-600" v-if="form.errors.hall">
                        {{ form.errors.hall }}
                    </div>
                </div>
                <div class="space-y-2">
                    ...
                    <div class="text-sm text-red-600" v-if="form.errors.show">
                        {{ form.errors.show }}
                    </div>
                </div>
                <div class="space-y-4">
                ...
                <Button type="submit" :disabled="form.processing"
                    >Upraviť kultúrnu udalosť</Button
                >
            </form>
        </div>
    </AppLayout>
